...
class HydraulicRotationsEnv(gym.Env):
    def __init__(self, size=64, checkpoints=None, solved=False, grid=None, extended_reward=True):
        self.size = size
    ...
    def step(self, action: np.ndarray):
        ...
        if self.extended_reward:
            ...
            n = hash(next_state.tostring())
            if n in self.encountered:
                score -= 100

            self.encountered.add(hash(next_state.tostring()))
...
if __name__ == "__main__":
    ...
    env = HydraulicRotationsEnv(10, 2, solved=True)
    env.reset()

    print("EQUALITY TESTING")
    print(env.render())
    assert env.render() == env.render()
